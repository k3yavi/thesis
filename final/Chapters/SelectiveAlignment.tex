...
different pipelines. \note{However, we explored, manually, some of
...
\note{These observations led us to combine information from both
...
\note{While other complex alignment scenarios may occur, these oracle
...
\note{In terms of Spearman correlation between all methods, we observed the
...
\note{Finally, while one would not typically regard any of the
...
\note{The results, visualized using UpSetR~\cite{conway2017upsetr} and presented
...
transcriptome being used for quantification. \note{Another effect not
...
level (as discussed in~\Cref{sec:DE}). \note{It also suggests that there is practical
...
\note{the observations made in this manuscript about the sensitivity
