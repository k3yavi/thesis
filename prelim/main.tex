...
for fast and accurate tools to analyze the generated data. Both bulk and single cell sequencing


The first step of many sequencing analyses requires us to solve the problem of \ra. When 
reads are aligned to a collection of reference sequences that share a substantial amount 
of sub-sequence (near or exact repeats), a single read can have many potential alignments, 
and considering all such alignment can be crucial for downstream analysis like quantification.
This study discusses the evolution of the tools developed to handle the problem of \ra for 
next generation sequencing, their impact on RNA-seq analysis, introduce a novel concept, 
\qm, and an efficient algorithm implementing this approach called \rapmap which maps RNA-seq 
\reads (sequences) to the reference sequence(s) (transcriptome). By attempting only to report
the potential loci of origin of a sequencing read, and not the base-to-base alignment by which 
it derives from the reference, \rapmap is capable of \textit{mapping} sequencing reads to a 
target transcriptome substantially faster than existing alignment tools. The algorithm we
employ to implement \qm uses several efficient data structures and takes advantage of the 
special structure of shared sequence prevalent in transcriptomes to rapidly provide 
highly-accurate mapping information.

Bulk RNA-sequencing is an established method to perform genome-wide quantification of gene 
...
Finally, we propose to study the effectiveness of both our methods and extend it further.
\qm as concept is a novel but it trades-off speed with accuracy, which shows in 
\textit{relatively} complicated datasets. In our preliminary study for the extention of 
\qm, we extensively explore the idea of selectively aligning (SA) reads based on
their genotypic nature and optimize \rapmap for efficient dynamic dataset dependent speed
and accurcay tradeoff. \Alevin utilizes \rapmap for transcriptome mapping and improved
mapping would be directly reflected on the performance of \alevin. Moreover, we propose
a principled approach and study the affect of further extending \alevin's UMI 
deduplication algorithm to include unspliced RNA.
...
grow both personally and professionally. A big fraction of this is due to my advisor, 
Rob Patro, whom I'd like to thank for his patience, guidance and encouragement for letting 
me explore the field of computational biology has I did not had any previous experience. 
Working with him, has been very exciting; He taught me how to do science and his vision 
...
important contributions through various lectures, talks and feedback on this thesis has 
...
and Fatemeh Almodaresi made Combine-Lab an interesting place to work and provide
insightful discussions. I couldn't imagine working through all the paper-deadlines
...
Specially my father, Y K Srivastav, who guided from my childhood to learn and motivated me 
to think out-of-box. I'd like to thank my mother Ritima Srivastav, brother Prakhar Srivastava,
late grand parents, Govind Prasad Srivastava, Savitri Srivastava and all cousins brother and
...
possible by a $\$1.4$M National Science Foundation grant (\#1531492).
